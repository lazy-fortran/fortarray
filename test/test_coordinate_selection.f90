...
contains
    ...
    subroutine test_sel_single_value()
        ...
        ! Create coordinate with proper error handling
        ...
        result = var%sel_point("x", 30.0_real64)
    ...
    subroutine test_sel_multiple_values()
        ...
        result = var%sel_point("x", 20.0_real64)
        if (result%n_elements /= 1) then
            test_passed = .false.
            write(error_unit,'(A)') "Single value selection wrong size"
    ...
    subroutine test_sel_range()
        ...
        result = var%sel_range("x", 25.0_real64, 75.0_real64)
    ...
    subroutine test_sel_nearest()
        ...
        result = var%sel_point("x", 27.0_real64, method="nearest")
        ...
        result = var%sel_point("x", 40.0_real64, method="nearest")
    ...
    subroutine test_sel_multidim()
        ...
        ! Create y coordinate with proper error handling
        ...
        var%has_coord(1) = .true.
        var%has_coord(2) = .true.
        
        ! Select on both dimensions (first x, then y)
        result = var%sel_point("x", 20.0_real64)
        result = result%sel_point("y", 200.0_real64)
        ...
        result = var%sel_point("y", 300.0_real64)
    ...
    subroutine test_sel_between()
        ...
        ! Create time coordinate with proper error handling
        call create_coordinate(time_coord, size(time_vals), "real64", i)
        time_coord%name = "time"
        time_coord%values_r64 = time_vals
        var = new_array(data, name="timeseries", dim_names=["time"])
        if (allocated(var%coords)) deallocate(var%coords)
        if (allocated(var%has_coord)) deallocate(var%has_coord)
        allocate(var%coords(1), var%has_coord(1))
        var%coords(1) = time_coord
        var%has_coord(1) = .true.
        
        ! Select between values (inclusive)
        result = var%sel_range("time", 3.5_real64, 7.5_real64)
    ...
    subroutine test_sel_tolerance()
        ...
        result = var%sel_point("x", 2.1_real64)
        ...
        result = var%sel_point("x", 2.5_real64)
    ...
    subroutine test_sel_string_coords()
        ...
        ! Create label coordinate with proper error handling
        call create_coordinate(label_coord, 4, "real64", i)
        label_coord%name = "x"
        label_coord%values_r64 = [1.0_real64, 2.0_real64, 3.0_real64, 4.0_real64]
        var = new_array(data, name="categorical", dim_names=["x"])
        if (allocated(var%coords)) deallocate(var%coords)
        if (allocated(var%has_coord)) deallocate(var%has_coord) 
        allocate(var%coords(1), var%has_coord(1))
        var%coords(1) = label_coord
        var%has_coord(1) = .true.
        
        result = var%sel_point("x", 2.0_real64)  ! Would be "medium" with string coords
    ...
    subroutine test_sel_performance()
        ...
        y_coord%name = "y"
        y_coord%values_r64 = y_vals
        ! Create z coordinate with proper error handling
        ...
        var%has_coord(1) = .true.
        var%has_coord(2) = .true.
        var%has_coord(3) = .true.
        
        ! Time coordinate selection
        call cpu_time(start_time)
        result = var%sel_point("x", 50.0_real64)
        result = result%sel_point("y", 50.0_real64)
        result = result%sel_point("z", 25.0_real64)
    ...
    subroutine test_sel_edge_cases()
        ...
        result = var%sel_point("x", 100.0_real64)
    ...
    subroutine test_isel_vs_sel()
        ...
        result_sel = var%sel_point("x", 3.0_real64)
        result_isel = var%isel_point("x", 3)  ! 1-based indexing
    ...
    subroutine test_sel_method_parameter()
        ...
        ! Exact match (default)
        result = var%sel_point("x", 3.0_real64, method="exact")
        if (result%n_elements /= 0) then
            test_passed = .false.
        ...
        ! Nearest
        result = var%sel_point("x", 3.0_real64, method="nearest")
        ...
        result = var%sel_point("x", 3.0_real64, method="ffill")
        if (result%n_elements /= 1 .or. &
            abs(result%data%values_r64(1) - 2.0_real64) > 1e-10) then
            test_passed = .false.
        ...
        result = var%sel_point("x", 3.0_real64, method="bfill")
        if (result%n_elements /= 1 .or. &
