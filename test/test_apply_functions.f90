...
contains
    ...
    subroutine test_apply_vectorized_operations()
        ...
        if (result%n_elements /= 6) then
            ...
            do data(1) = 1, 6  ! reuse data variable as counter
                if (abs(result%data%values_r64(int(data(1))) - data(1)) > 1e-10) then
                    test_passed = .false.
                    write(error_unit,'(A,I0)') "Sqrt result incorrect at position: ", int(data(1))
                    exit
                end if
            end do
        end if
