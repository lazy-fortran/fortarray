module foxel_lazy_evaluation
    ...
    ! Public types
    public :: computation_graph_t, lazy_variable_t, operation_node_t
    ...
    type :: lazy_variable_t
        type(computation_graph_t), pointer :: graph => null()
        integer :: variable_id
        logical :: is_computed
        logical :: use_chunking
        integer :: chunk_size
        type(variable_t) :: cached_result
    end type lazy_variable_t
...
contains
    ...
    subroutine add_operation(graph, op_name, var1, var2_or_scalar)
        ...
        type(variable_t), intent(in) :: var1
    ...
    function compute(lazy_var) result(result_var)
        type(lazy_variable_t), intent(inout) :: lazy_var
    ...
    subroutine execute_operation(graph, op_idx)
        ...
        type(operation_node_t) :: op
        type(variable_t) :: input1, input2, result
    ...
    function get_result_variable(lazy_var) result(var)
        type(lazy_variable_t), intent(in) :: lazy_var
        type(variable_t) :: var
...
end module foxel_lazy_evaluation