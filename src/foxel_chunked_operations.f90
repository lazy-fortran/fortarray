module foxel_chunked_operations
    use foxel_types
    use foxel_storage
    ...
    type :: chunk_cache_t
        ...
        type(variable_t), dimension(:), allocatable :: cached_chunks
...
contains
    ...
    function create_chunk_iterator(var, chunk_size, chunk_shape) result(iterator)
        type(variable_t), target, intent(in) :: var
        integer, intent(in), optional :: chunk_size
        integer, dimension(:), intent(in), optional :: chunk_shape
    ...
    function apply_chunked(op, var, func) result(result)
        ...
        type(chunk_iterator_t) :: iterator
        type(variable_t) :: chunk, processed_chunk
        type(variable_t), dimension(:), allocatable :: chunks
    ...
    function apply_function_to_chunk(chunk, func) result(result)
        type(variable_t), intent(in) :: chunk
        interface
            function func(x) result(y)
                use iso_fortran_env, only: real64
                real(real64), intent(in) :: x
    ...
    function compute_out_of_core(var, operation, config) result(result)
        ...
        type(chunk_iterator_t) :: iterator
        type(variable_t) :: chunk, chunk_result
...
end module foxel_chunked_operations