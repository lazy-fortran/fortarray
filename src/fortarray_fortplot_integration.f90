module fortarray_fortplot_integration
    use iso_fortran_env, only: int32, int64, real32, real64, error_unit
    use fortarray_types
    use fortarray_storage
    use fortarray_datasets
    ! use fortarray_netcdf, only: delete_file  ! Not exported from fortarray_netcdf
...
contains
    ...
    subroutine scatter_plot(var_x, var_y, options)
        type(fortarray_t), intent(in) :: var_x, var_y
        type(plot_options_t), intent(in), optional :: options
    ...
    subroutine plot_with_errorbars(var, errors, options)
        type(fortarray_t), intent(in) :: var, errors
        type(plot_options_t), intent(in), optional :: options
    ...
    subroutine plot_with_fill(var_upper, var_lower, options)
        type(fortarray_t), intent(in) :: var_upper, var_lower
        type(plot_options_t), intent(in), optional :: options
    ...
    subroutine variable_savefig(var, filename, dpi)
        ...
        write(99,'(A)') "# Stub plot file created by fortarray"
        close(99)
...
end module fortarray_fortplot_integration