module fortarray_parallel_computing
...
contains
    ...
    function broadcast_add_parallel(small_var, large_var) result(output)
        type(fortarray_t), intent(in) :: small_var, large_var
        type(fortarray_t) :: output
        integer :: i, n, small_n
    ...
    function where_parallel(condition, true_vals, false_vals) result(output)
        type(fortarray_t), intent(in) :: condition, true_vals, false_vals
        type(fortarray_t) :: output
        integer :: i, n
    ...
    function parallel_partition(var, n_partitions) result(partitions)
        ...
        type(fortarray_t), dimension(:), allocatable :: partitions
        allocate(partitions(1))
        partitions(1) = var
    end function parallel_partition
    
    function parallel_merge(partitions) result(merged)
        type(fortarray_t), dimension(:), intent(in) :: partitions
        type(fortarray_t) :: merged
        merged = partitions(1)
...
end module fortarray_parallel_computing